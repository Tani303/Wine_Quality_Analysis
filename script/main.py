...
# Set paths for data and results
data_path = './data/winequality-red.csv'
model_path = './results/models/wine_quality_model.joblib'  # Changed to .joblib for consistency
plots_path = './results/plots/'
...
def plot_advanced_correlations(df, output_path):
    """Plot advanced correlation matrix with annotated values and better color scaling."""
    corr = df.corr()
    mask = np.triu(np.ones_like(corr, dtype=bool))
    cmap = sns.diverging_palette(230, 20, as_cmap=True)
    plt.figure(figsize=(10, 8))
    sns.heatmap(corr, mask=mask, cmap=cmap, vmax=1, vmin=-1, annot=True, square=True, linewidths=.5, cbar_kws={"shrink": .8})
    plt.title('Advanced Feature Correlation Matrix')
    plt.savefig(output_path)
    plt.close()

def plot_feature_quality_correlations(df, output_path):
    """Plot the correlations of features with wine quality."""
    # Calculate correlation matrix
    corr = df.corr()
    
    # Sort the correlations with wine quality in descending order
    wine_quality_correlations = corr['quality'].drop('quality').sort_values(ascending=True)
    
    # Create a horizontal bar plot for all features
    plt.figure(figsize=(10, 12))
    sns.barplot(x=wine_quality_correlations.values, y=wine_quality_correlations.index, palette='coolwarm')
    plt.xlabel('Correlation with Wine Quality')
    plt.ylabel('Features')
    plt.title('Correlation of Features with Wine Quality')
    plt.savefig(output_path + 'feature_quality_correlations.png')
    plt.close()
...
def feature_selection(X_train, X_test, y_train):
    ...
    model.fit(X_train, y_train)
    sfm = SelectFromModel(model, threshold='median', prefit=True)
    feature_indices = sfm.get_support(indices=True)  # Get the indices of selected features
    X_train_selected = X_train.iloc[:, feature_indices]
    X_test_selected = X_test.iloc[:, feature_indices]
    return X_train_selected, X_test_selected
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    print(f'Mean Squared Error: {mse}')
    print(f'R2 Score: {r2}')
    # joblib.dump(model, model_path)  # Removed joblib dump as per requirement

def main():
    df = load_data(data_path)
    df = preprocess_data(df)
    plot_quality_distribution(df, os.path.join(plots_path, 'quality_distribution.png'))
    plot_advanced_correlations(df, os.path.join(plots_path, 'advanced_correlation_matrix.png'))
    plot_pairplot(df, os.path.join(plots_path, 'pairplot.png'))
